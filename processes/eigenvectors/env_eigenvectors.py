...
def cluster_features_by_loadings(
    ...
    if n_features == 0:
        return pd.DataFrame(columns=["feature", "feature_cluster"])
    ...
    n_clusters_eff = max(1, min(int(n_clusters), n_features))
    if n_features == 1 or n_clusters_eff == 1:
...
def run_pc_selection(
    ...
    requested_clusters = int(cfg.pcsel_feature_clusters)
    available_features = int(loadings_df.shape[0])
    if requested_clusters > available_features:
        print(
            f"[WARN] pcsel_feature_clusters={requested_clusters} exceeds available "
            f"features={available_features}; using {available_features}."
        )

    feature_clusters = cluster_features_by_loadings(
        loadings=loadings_df,
        pcs_to_use=pcs_parallel,
        n_clusters=min(requested_clusters, max(1, available_features)),
        random_state=cfg.random_state,
    )
...
def main() -> None:
    ...
    sparse_corr_df = pd.DataFrame(
        sparse_corr_rows,
        columns=["feature", "PC", "spearman_r", "n_samples_used", "coverage"],
    )
    ...
    sparse_corr_mapped = sparse_corr_df[
        sparse_corr_df["feature"].isin(BIOCHEM_COLOR_MAP.keys())
    ].copy()
